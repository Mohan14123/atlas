...
export default function Dashboard() {
  ...
  // Throughput data from backend
  const throughputData = metrics?.jobs.throughput_history || [];
  ...
  const workerData = metrics?.workers.list?.map((w: any) => ({
    name: w.hostname,
    utilization: w.concurrency > 0 ? (w.active_jobs / w.concurrency) : 0
  })) || [];
